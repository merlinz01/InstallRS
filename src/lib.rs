...
//! `#[doc(hidden)]` (e.g. [`EmbeddedEntry`], [`verify_payload`],
//! [`Installer::new`], [`Installer::install_main`], the `Source` inner
//! field) are part of the contract between this crate and the code
//! generated by the `installrs` CLI — **not** a public API. They may
//! change in any release, including patch releases. User code should
//! never call them; the generated installer crate pins
//! `installrs = "=X.Y.Z"` so the CLI and runtime always match.
