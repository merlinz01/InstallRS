...
#[doc(hidden)]
pub mod __private {
    /// Install a PNG byte-slice as the default window icon for every
    /// GTK window and dialog the wizard creates. No-op on Windows
    /// (icons come from the embedded `.ico` resource) and on builds
    /// without a GTK backend. The build tool emits a call to this at
