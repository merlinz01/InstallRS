...
/// Internal items called from code generated by the `installrs` CLI.
/// Not part of the public API — stability is governed by the
/// CLI-runtime version pin (`installrs = "=X.Y.Z"`), not semver.
...
use embedded::EmbeddedEntry;
#[cfg(test)]
use embedded::{DirChild, DirChildKind};
use options::CmdOption;
use progress::ProgressState;
#[cfg(test)]
use source::source_path_hash_const;
...
impl Installer {
    ...
    ///
    /// `help` is a one-line description shown in `--help` output. Pass
    /// `""` to omit the description (the option still appears in the
    /// list).
    ///
    ...
    /// A handle to the shared cancellation flag. Cloning yields another
    /// handle to the same flag; flipping any handle trips
    /// [`check_cancelled`](Self::check_cancelled) and aborts the
    /// next file/dir/mkdir/remove op. The wizard's Cancel button and
    /// the Ctrl+C handler both write through this.
...
mod tests {
    ...
    #[test]
    fn cli_help_text_lists_builtins_user_options_and_components() {
        let mut i = make_bare_installer();
        i.add_component("core", "Core files", "Required bits", 5)
            .required();
        i.add_component("docs", "Documentation", "User guide", 1);
        i.add_option("config", OptionKind::String, "Path to a config file");
        i.add_option("port", OptionKind::Int, "Listen port");
        i.add_option("verbose", OptionKind::Flag, "");
        let text = i.help_text(&["my-installer".into()]);
        assert!(text.contains("USAGE: my-installer"));
        assert!(text.contains("--help, -h"));
        assert!(text.contains("--headless"));
        assert!(text.contains("--config <STRING>"));
        assert!(text.contains("Path to a config file"));
