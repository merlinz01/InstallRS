...
impl Installer {
    ...
    /// Raw parsed value for an option, or `None` if not set. Internal:
    /// user code should reach for the typed [`Self::option`] instead;
    /// this is exposed only because the wizard's custom-page widgets
    /// commit their current values back without knowing the static type.
    #[allow(dead_code)] // Used only by the GUI backends, both feature-gated.
    pub(crate) fn option_value(&self, name: &str) -> Option<&OptionValue> {
        let name = name.trim_start_matches('-');
        self.option_values.get(name)
    }

    /// Store (or overwrite) a parsed option value directly. Internal:
    /// the wizard calls this when a custom-page widget commits its
    /// current value on forward navigation. User code that wants to
    /// write a typed value should call [`Self::set_option`].
    #[allow(dead_code)] // Used only by the GUI backends, both feature-gated.
    pub(crate) fn set_option_value(&mut self, name: &str, value: OptionValue) {
        let name = name.trim_start_matches('-').to_string();
        self.option_values.insert(name, value);
    }

    /// Clone of the full parsed-options map. Internal: the wizard uses
    /// it to pre-fill custom-page widgets from already-set option
    /// values (whether from the CLI or a previous wizard run).
    #[allow(dead_code)] // Used only by the GUI backends, both feature-gated.
    pub(crate) fn option_values_snapshot(&self) -> std::collections::HashMap<String, OptionValue> {
...
mod tests {
    ...
    fn cli_user_options_parse_and_typed_read() {
        let mut i = make_bare_installer();
        i.add_option("config", OptionKind::String, "");
        i.add_option("port", OptionKind::Int, "");
        i.add_option("verbose", OptionKind::Flag, "");
        i.add_option("fast", OptionKind::Bool, "");
        let args = vec![
            "installer".into(),
            "--config".into(),
            "/etc/my.conf".into(),
        ...
        i.process_commandline_from(&args).unwrap();
        assert_eq!(
            i.option::<String>("config").as_deref(),
            Some("/etc/my.conf")
        );
        assert_eq!(i.option::<i64>("port"), Some(8080));
        assert_eq!(i.option::<i32>("port"), Some(8080));
        assert_eq!(i.option::<bool>("verbose"), Some(true));
        assert_eq!(i.option::<bool>("fast"), Some(true));
    }
    ...
    fn cli_flag_absent_is_false() {
        ...
        assert_eq!(i.option::<bool>("verbose"), Some(false));
    }
